...
function DataDropdown({ isVisible, datas, toggleValue, closeCard }) {
  return (
    <section
      className={`data-dropdown-section ${isVisible ? "active" : ""}`}
    >
        <nav className="data-dropdown-close">
            <div onClick={() => {closeCard()}}></div>
        </nav>
      <div className="data-dropdown-content">
        <hr />
          {datas.map((data, i) => (
