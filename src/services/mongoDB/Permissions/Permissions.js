...
export const createPermission = async (
  ...
  const response = await api.post(`/permissoes/inserir`, {
    idEmpresa,
    nomePermissao,
    idFuncionario,
  });
...
export const editPermission = async (
  ...
  const response = await api.patch(`/permissoes/atualizar/${id}`, {
    idEmpresa,
    nomePermissao,
    idOperario,
  });
...
export const insertWorkerInPermission = async (id, wokersId) => {
  const response = await api.patch(
    `/permissoes/adicionar-ids-operario/${id}`,
    wokersId
  );
  return response.data;
};

export const deletePermById = async (id) => {
    const response = await api.delete(
    `/permissoes/excluir/${id}`
  );
  return response.data;
}